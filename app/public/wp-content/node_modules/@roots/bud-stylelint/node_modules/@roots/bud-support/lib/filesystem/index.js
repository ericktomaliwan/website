...
let filesystem = new Filesystem(process.cwd());
export const get = (basedir) => {
    if (typeof basedir !== `string`)
        throw BudError.normalize(`filesystem not initialized. basedir arg required for initialization.`, { thrownBy: import.meta.url });
    if (filesystem && currentDirectory === basedir)
        return filesystem;
     ...
     */
    const modifiedBaseDirectory = argv.includes(`--basedir`) || argv.includes(`--cwd`);
    if (isMainThread && modifiedBaseDirectory)
