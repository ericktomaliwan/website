 ...
 */
export class FS extends Filesystem {
    _app;
    /**
     * JSON
     *
    ...
    upload(options) {
        this.app.after(async () => {
            if (!this.s3.config.credentials) {
                throw BudError.normalize(`S3 is not configured. See https://bud.js.org/reference/bud.fs/s3`);
            }
            const s3Path = (path) => destination ? join(destination, path) : path;
            const { destination, files, keep, source } = {
                destination: options?.destination,
                files: options?.files ?? `**/*`,
                keep: isNumber(options?.keep) || isBoolean(options?.keep)
                    ? options?.keep
                    : 5,
            ...
            await globby(files, { cwd: source }).then(async (files) => {
                const descriptions = await Promise.all(files.map(async (file) => {
                    this.logger.info(`Attempting to read ${file}:`, join(source, file));
                ...
                await Promise.all([...new Set(stale)]
                    .flatMap(([, value]) => value)
                    .filter(key => !entries.some(([_, value]) => Array.isArray(value) && value.includes(key)))
                    .map(async (key) => {
                    if (!isString(key))
                        return;
