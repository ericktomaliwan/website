...
const get = async (fromDirectory) => {
    ...
    await Promise.all([
        ...
        fs
            ...
            .then(res => files.push(...((res?.filter(Boolean) ?? []).map(file => join(`bud`, file)) ?? []))),
        fs
            .list(join(basedir, `.bud`))
            .then(res => files.push(...((res?.filter(Boolean) ?? []).map(file => join(`.bud`, file)) ?? []))),
    ]).catch(origin => {
...
async function getFileInfo(filename) {
    ...
    if (file.type === `module`) {
        file.module = async () => {
            ...
            const tmpfile = join(basedir, file.dir, `.${file.name}${file.ext.replace(/(.*)ts$/, `$1mjs`)}`);
