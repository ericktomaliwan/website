...
export const Versions = ({ packages, }) => {
    ...
    return (_jsx(LabelBox, { label: "Version compatibility", children: packages.map((result, i) => {
            ...
            if (result.error) {
                renderError(BudError.normalize(`${result.signifier} is not installed at the same version as @roots/bud (required: ${result.requiredVersion}, installed: ${result.packageVersion}).
            ...
            return (_jsx(Box, { children: _jsx(Package, { ...result }) }, i));
        }) }));
};
