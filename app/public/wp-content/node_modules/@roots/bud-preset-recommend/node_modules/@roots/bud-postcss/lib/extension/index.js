...
let BudPostCss = class BudPostCss extends BudPostCssOptionsApi {
    ...
    async register({ build, context, hooks, }) {
        ...
        build.rules.css.setUse((items = []) => items.includes(`postcss`) ? items : [...items, `postcss`]);
        build.rules[`css-module`]?.setUse((items = []) => items.includes(`postcss`) ? items : [...items, `postcss`]);
        ...
        const postcssImport = await this.resolve(`postcss-import`, import.meta.url);
        postcssImport &&
