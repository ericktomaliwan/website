...
export const processConfigs = async function () {
    ...
    const processConfig = async (file) => {
        ...
        await configuration.run(file).catch(error => {
            throw BudError.normalize(`Error parsing ${file.name}: ${BudError.normalize(error).message}`, {
                file,
                origin: error,
    ...
    await this.executeServiceCallbacks(`config.after`).catch(origin => {
        this.catch(BudError.normalize(`Error running hook: config.after`, {
    ...
    await Promise.all(Object.values(this.children).map(async (child) => {
        await child.tapAsync(async (child) => {
            await child
                ...
                .catch(origin => {
                this.catch(BudError.normalize(`Error executing service callback: config.after`, {
                    details: `This is likely an error in the project bud configuration.`,
                    docs: new URL(`https://bud.js.org/learn/config/files/bud.config`),
