...
export default class BudUpgradeCommand extends BudCommand {
     ...
     * {@link Command.paths}
     */
    static paths = [[`upgrade`], [`version`, `set`]];
    /**
     * {@link Command.usage}
     */
     ...
     * Browserslist option
     */
    browserslistUpdate = browserslistUpdate;
    /**
     * Package manager option
     */
    pm = Option.String(`--pm`, undefined, {
     ...
     * Registry option
     */
    registry = Option.String(`--registry`, `https://registry.npmjs.org`, {
    ...
    async execute() {
        ...
        if (this.pm === `yarn`) {
            if (this.registry !== `https://registry.npmjs.org`) {
                logger.warn(`Yarn berry does not support custom registries set by CLI. Ignoring --registry flag; set your custom registry in \`.yarnrc.yml\``);
            ...
            if (await this.bud.fs.exists(this.bud.path(`.yarnrc.yml`))) {
                await this.bud.fs.yml
                    ...
                    if (yarnrc[`npmRegistryServer`]) {
                        ...
                        logger.log(`registry set to`, this.registry, `(setting sourced from .yarnrc.yml)`);
                    }
                })
                    .catch(error => {
     ...
     * from the registry
     */
    async getUpgradeableDependencies(type) {
        if (typeof this.upgradeable[type] !== `undefined`) {
            logger.log(`using cached results for`, type, `\n`, this.upgradeable[type]);
        ...
        const communityDependencies = this.findCandidates(type).filter(signifier => !signifier.startsWith(`@roots/`));
        await Promise.all(communityDependencies.map(async (signifier) => {
             ...
             */
            const match = manifests.find(({ bud, version, }) => {
             ...
             */
            if (!match) {
                logger.warn(`No version of ${signifier} found which is compatible with ${this.version}. Installing ${signifier}@latest.`);
        ...
        if (this.upgradeable[type] && this.upgradeable[type].length) {
            logger.log(`discovered upgradeable`, type, `:\n`, this.upgradeable[type]);
        }
        return this.upgradeable[type];
    }
    async upgrade(type, flags = []) {
        ...
        await this.$(this.bin, [this.subcommand, ...signifiers, ...flags], undefined, () => { }).catch(error => {
            logger.error(error);
        });
    }
}
__decorate([
    bind
], BudUpgradeCommand.prototype, "doRegistryRequest", null);
__decorate([
    bind
], BudUpgradeCommand.prototype, "findCandidates", null);
__decorate([
    bind
], BudUpgradeCommand.prototype, "getUpgradeableDependencies", null);
__decorate([
    bind
], BudUpgradeCommand.prototype, "upgrade", null);
