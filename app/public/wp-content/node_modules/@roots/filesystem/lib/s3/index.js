...
export class S3 {
    ...
    async read(key, raw = false) {
        const streamToString = async ({ Body: stream }) => new Promise((resolve, reject) => {
            const chunks = [];
            stream
        ...
        try {
            const request = await client.send(GetObject);
            if (!request)
                return undefined;
            if (raw)
                return request;
    ...
    async write(...params) {
        ...
        if (typeof params[0] !== `string`)
            ...
            Object.assign(putProps, { Body: params[1], Key: params[0] });
        if (!putProps.Key)
