...
export const baseDir = ([basedir]) => {
    if (!isAbsolute(basedir)) {
        throw BudError.normalize(`bud.setPath: when only one parameter is provided, it must be an absolute path.`, {
...
export const stringPair = ([key, value]) => {
    if (!isString(value)) {
        throw BudError.normalize(`bud.setPath: tried to set path with a non-string value`, {
    ...
    if (!isString(key)) {
        throw BudError.normalize(`bud.setPath: Tried to set path with a non-string key`, {
