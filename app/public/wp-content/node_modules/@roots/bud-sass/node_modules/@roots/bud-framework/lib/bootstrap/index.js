 ...
 */
export const bootstrap = async (bud) => {
    bud
        .set(`fs`, new FS(() => bud))
        .set(`module`, bud.isRoot
    ...
    await Promise.all([...bud.context.services]
    ...
    Object.entries(lifecycle).map(([eventHandle, callbackName]) => [...services]
        ...
        logger
            ...
            .log(`Registering callback:`, `${instance.constructor.name}.${callbackName}`);
        bud.hooks.action(eventHandle, instance[callbackName]);
    }));
    /**
 ...
 */
const filterServices = (app) => (signifier) => {
    if (!isString(signifier))
        return true;
    return ((app.isDevelopment || !DEVELOPMENT_SERVICES.includes(signifier)) &&
 ...
 */
const instantiateServices = (bud) => async (signifier) => {
    const Service = await bud.module.import(signifier);
    const value = new Service(() => bud);
    const handle = (value.constructor?.name
