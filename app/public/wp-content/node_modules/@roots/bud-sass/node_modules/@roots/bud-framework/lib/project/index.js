...
export default class Project extends Service {
    ...
    async compilerBefore(bud) {
        ...
        await bud.fs
            .write(bud.path(`@storage`, bud.label, `debug`, `profile.yml`), getData(bud))
            .catch(this.logger.error)
            .finally(() => {
            this.logger.log(`profile.yml written to disk`);
        });
        await bud.fs
            .write(bud.path(`@storage`, bud.label, `debug`, `build.config.yml`), bud.hasChildren
            ? Object.entries(bud.children).reduce((acc, [label, bud]) => ({ ...acc, [label]: bud.build.config }), {})
            : bud.build.config)
            .catch(this.logger.error)
            .finally(() => {
...
__decorate([
    bind
], Project.prototype, "compilerBefore", null);
